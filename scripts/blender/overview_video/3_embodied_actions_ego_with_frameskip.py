...
end_frame = 744
# end_frame = 627
frame_skip = 3  # Render every 3rd frame (571, 574, 577, ...)
...
cam_locs, cam_rots = sample_camera_world_transforms(pov_camera_obj, frames_to_render)
...
disable_animations_except_camera_and_armature(pov_camera_obj, armature_obj)

# Bake camera keys so camera moves during rendering
print(f"\nBaking camera keys for {len(frames_to_render)} frames...")
bake_camera_keys(pov_camera_obj, frames_to_render, cam_locs, cam_rots)
...
# Output file path
basename = "embodied_actions_ego_with_frameskip"
...
print(f"\n" + "="*60)
print(f"RENDERING EMBODIED ACTIONS FROM POV CAMERA ({output_type})")
print("="*60)
...
try:
    ...
    clear_camera_keys(pov_camera_obj)
    
    # Restore animations
    print("Restoring animations...")
    restore_animations(pov_camera_obj)
