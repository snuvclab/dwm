...
# ------------------------------------------------------------------
# Apply character material (from blender_ego_static_with_agent.py)
# ------------------------------------------------------------------
body_mesh_name = "CC_Base_Body"
body_mesh_obj = bpy.data.objects.get(body_mesh_name)
if body_mesh_obj:
    print(f"\nApplying character material to '{body_mesh_name}'...")
    character_material = create_character_material(COLOR)
    body_mesh_obj.data.materials.clear()
    body_mesh_obj.data.materials.append(character_material)
    print(f"✓ Applied character material with color: {COLOR}")
else:
    print(f"Warning: Body mesh '{body_mesh_name}' not found. Character material will not be applied.")
