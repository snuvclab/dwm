...
def create_pose_material(index, total, frame_number, animation_tag=None, color_rgba=None):
    ...
    # Alpha increases over time: transparent (0.3) => opaque (1.0)
    # First frame (index=0) is most transparent, last frame (index=total-1) is most opaque
    alpha = 0.3 + (1.0 - 0.3) * (index / max(1, total - 1))
    ...
    if args.frames_list:
        ...
        for idx, frame_num in enumerate(frames_to_render):
            ...
            try:
                ...
                if color_ramp and idx < len(color_ramp):
                    color_override = color_ramp[idx]
                material = create_pose_material(idx, total, frame_num, animation_name, color_override)
                ...
                print(f"Created body snapshot for frame {frame_num}")
            except RuntimeError as err:
            ...
            for hand_obj, hand_name in [(hand_L_obj, "Hand_L"), (hand_R_obj, "Hand_R")]:
                if hand_obj:
                    ...
                    try:
                        ...
                        # Copy material from original hand object
                        if hand_obj.data.materials:
                            hand_snapshot.data.materials.clear()
                            hand_snapshot.data.materials.append(hand_obj.data.materials[0])
                        hand_snapshot.hide_render = False
                        hand_snapshot.hide_viewport = False
                        print(f"Created {hand_name} snapshot for frame {frame_num}")
                    except RuntimeError as err:
            ...
            if camera_obj:
                try:
                    ...
                    print(f"Created frustum snapshot for frame {frame_num}")
