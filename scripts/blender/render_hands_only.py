#!/usr/bin/env python3
"""
Blender script to render only hand meshes with white background.
Run this script in Blender after executing blender_ego_static_with_agent_fig2.py

Usage:
    blender your_file.blend --python render_hands_only.py
    OR
...
def render_hands_only(output_path=None):
    """
    Hide all objects except hand snapshots and render with white background.
    ...
    # 3. Show only hand snapshots
    hand_objects = []
    if collection:
        for obj in collection.objects:
            # Show hand snapshots (Hand_L or Hand_R in name)
            if "Hand_L" in obj.name or "Hand_R" in obj.name:
                if "R_18" in obj.name:
                    obj.hide_render = True
                    continue
                if "L_18" in obj.name:
                ...
                hand_objects.append(obj.name)
                print(f"  Showing: {obj.name}")
    
    if not hand_objects:
        print("WARNING: No hand snapshots found in collection!")
        print("Objects in collection:")
        for obj in collection.objects:
            print(f"  - {obj.name}")
        return False
    
    print(f"Found {len(hand_objects)} hand snapshot objects")
    
    # 3.5. Remove transparency from hand materials
    print("\nRemoving transparency from hand materials...")
    for obj_name in hand_objects:
            ...
            for mat in obj.data.materials:
                if mat and mat.use_nodes:
                    ...
                    if bsdf and "Alpha" in bsdf.inputs:
                        ...
                        print(f"  Set alpha to 1.0 for {obj_name} material: {mat.name}")
                    # Disable transparency blending
                    mat.blend_method = 'OPAQUE'
                    mat.shadow_method = 'OPAQUE'
    
    # 4. Also show original hand objects if they exist (optional)
    for hand_name in ["CC_Hand_L", "CC_Hand_R"]:
        hand_obj = bpy.data.objects.get(hand_name)
        if hand_obj:
            hand_obj.hide_render = True
            hand_obj.hide_viewport = False
            print(f"  Also showing original: {hand_name}")
    ...
    # Set white background - use high strength for bright white background
    # Light bounces are set to 0, so high strength won't affect hand meshes via indirect lighting
    bg_node.inputs["Color"].default_value = (1.0, 1.0, 1.0, 1.0)
    bg_node.inputs["Strength"].default_value = 1.0  # High strength for bright white background
    ...
    # 5.6. Reduce indirect lighting from World shader to prevent hand meshes from brightening
    if scene.cycles:
        ...
        print("Reduced light bounces to prevent background from affecting hand meshes")
    ...
    if output_path:
        ...
        # Use default or create hands_output directory
        blend_filepath = bpy.data.filepath
        if blend_filepath:
            blend_dir = os.path.dirname(blend_filepath)
            output_dir = os.path.join(blend_dir, "hands_output")
            os.makedirs(output_dir, exist_ok=True)
            scene.render.filepath = os.path.join(output_dir, "hands_only")
        else:
            scene.render.filepath = "//hands_only"
    ...
    print(f"Rendered {len(hand_objects)} hand snapshot objects")
    print("="*60)
...
def main():
    ...
    success = render_hands_only(output_path)
