#!/bin/bash
#SBATCH --job-name=trumans_aether_training_perframe
#SBATCH --nodes=1
#SBATCH --gpus=2
#SBATCH --partition=batch
#SBATCH --output=out/trumans_aether_perframe_%j.out
#SBATCH --error=out/trumans_aether_perframe_%j.err
...
output_dir="outputs/trumans_full_aether_w_bodypose_perframe/"

# Launch training with Aether on Trumans dataset (PER-FRAME SMPL conditioning)
cmd="accelerate launch --config_file $ACCELERATE_CONFIG_FILE --gpu_ids $GPU_IDS training/aether/aether_smplx_lora.py \
    ...
    --smpl_conditioning_type per_frame \
    --use_empty_prompts"

echo "===== Training Aether on Trumans Dataset (PER-FRAME SMPL) ====="
...
echo "GPUs: $GPU_IDS"
echo "SMPL Conditioning: PER-FRAME"
echo ""
