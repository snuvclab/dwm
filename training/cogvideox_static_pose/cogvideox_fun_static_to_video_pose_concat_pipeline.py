...
class CogVideoXFunStaticToVideoJointGenerationPipeline(CogVideoXFunStaticToVideoPipeline):
    r"""
    Pipeline for text-to-video generation using CogVideoX with static and hand pose conditions,
    supporting joint generation of two videos simultaneously.
    
    This model inherits from [`CogVideoXFunStaticToVideoPipeline`]. It extends the base pipeline
    to support joint generation of two videos using a transformer with doubled output dimension.
    ...
    Args:
        ...
        transformer ([`CogVideoXFunTransformer3DModelWithJointGeneration`]):
            A text conditioned transformer with doubled output dimension for joint generation.
        scheduler ([`SchedulerMixin`]):
            A scheduler to be used in combination with `transformer` to denoise the encoded video latents.
    """
    ...
    def from_pretrained(cls, pretrained_model_name_or_path=None, 
        ...
        if base_model_name_or_path is not None:
            print(f"🔧 Loading pipeline components (tokenizer, text_encoder, vae, scheduler) from base model: {base_model_name_or_path}")
            # Load the original CogVideoX-Fun pipeline from base model to get non-transformer components
            original_pipeline = super(CogVideoXFunStaticToVideoPipeline, cls).from_pretrained(base_model_name_or_path, *args, **kwargs)
        ...
        if transformer is None:
            ...
            # Create or load joint generation transformer
            print(f"🔧 Creating/loading VideoX-Fun joint generation transformer with {condition_channels} condition channels")
            ...
            if "torch_dtype" in kwargs:
                load_dtype = kwargs["torch_dtype"]
            print(f"🔧 Loading VideoX-Fun joint generation pipeline from {pretrained_model_name_or_path}")
            transformer = CogVideoXFunTransformer3DModelWithJointGeneration.from_pretrained(
                pretrained_model_name_or_path,
                subfolder="transformer",
                torch_dtype=load_dtype,
    ...
    def prepare_latents(
        ...
        # Always return static, hand video latents (can be None)
    ...
    def __call__(
        ...
        """
        Function invoked when calling the pipeline for joint generation.
        Returns two videos generated simultaneously.

        Examples:

        Returns:
            [`~pipelines.cogvideo.pipeline_cogvideox.CogVideoXFunPipelineOutput`] or `tuple`:
            If `return_dict` is True, returns a dict with 'frames' containing a list of two videos.
            Otherwise returns a tuple with two video tensors.
        """
        # Most of the logic is the same as parent class, but we need to handle the doubled output
        if isinstance(callback_on_step_end, (PipelineCallback, MultiPipelineCallbacks)):
            callback_on_step_end_tensor_inputs = callback_on_step_end.tensor_inputs
        ...
        # 5. Prepare latents (same as parent)
        if image is not None and static_videos is None:
            if isinstance(image, PIL.Image.Image):
                image_tensor = self.video_processor.preprocess(image, height, width, resize_mode="crop")
            elif isinstance(image, np.ndarray):
        ...
        if num_frames <= 0:
            num_frames = 1
        if init_video is not None and video_length > num_frames:
            logger.warning("The length of condition video is not right, the latent frames should be clipped to make it divisible by patch_size_t. ")
            video_length = num_frames
            if video is not None:
                video = video[:, :, :video_length]
        ...
        # For joint generation, prepare latents with doubled channels (like aether)
        # Prepare latents for first video
        latents_outputs = self.prepare_latents(
            batch_size=batch_size * num_videos_per_prompt,
            num_channels_latents=num_channels_latents,
            height=height,
        ...
        # Decode both videos
        if not output_type == "latent":
            video1 = self.decode_latents(latents_video1)
            video2 = self.decode_latents(latents_video2)
            video1 = rearrange(video1, "b c f h w -> b f h w c")
            video2 = rearrange(video2, "b c f h w -> b f h w c")
        else:
            video1 = latents_video1
            video2 = latents_video2
