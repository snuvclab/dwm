...
parser.add_argument("--video_path", type=str, required=True, help="Path to the text file containing validation video list")
parser.add_argument("--hand_path", type=str, required=True, help="Path to the text file containing prompts for validation videos")
parser.add_argument("--prompt", action='store_true', help="If set, do not use prompt during inference")
...
input_video,  _, _, _ = get_video_to_video_latent(args.video_path, video_length=video_length, sample_size=sample_size, validation_video_mask=None, fps=fps)
...
hand_video,  _, _, _ = get_video_to_video_latent(args.hand_path, video_length=video_length, sample_size=sample_size, validation_video_mask=None, fps=fps)
    ...
    with torch.no_grad():
        ...
        def save_results(idx):
            if not os.path.exists(save_path):
                os.makedirs(save_path, exist_ok=True)
            name = args.video_path.replace("/", "_").replace(".mp4", f"_{idx}.mp4")
