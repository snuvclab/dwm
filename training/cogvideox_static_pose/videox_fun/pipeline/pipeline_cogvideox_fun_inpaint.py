...
class CogVideoXFunInpaintPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def __call__(
        ...
        if hand_video is not None:
            hand_video_length = hand_video.shape[2]
            hand_video = self.image_processor.preprocess(rearrange(hand_video, "b c f h w -> (b f) c h w"), height=height, width=width) 
            hand_video = hand_video.to(dtype=torch.float32)
            hand_video = rearrange(hand_video, "(b f) c h w -> b c f h w", f=hand_video_length)
        else:
            hand_video = None
        ...
        if video_length > num_frames:
            ...
            hand_video = hand_video[:, :, :video_length] if hand_video is not None else None
        ...
        if hand_video is not None:
            bs = 1
            new_video = []
            hand_video = hand_video.to(latents.device, latents.dtype)
            with torch.no_grad():
                for i in range(0, hand_video.shape[0], bs):
                    video_bs = hand_video[i : i + bs]
                    video_bs = self.vae.encode(video_bs)[0]
                    video_bs = video_bs.sample()
                    new_video.append(video_bs)
            hand_video = torch.cat(new_video, dim = 0)
            hand_video = hand_video * self.vae.config.scaling_factor

            hand_video_latents = hand_video.repeat(batch_size // hand_video.shape[0], 1, 1, 1, 1)
            hand_video_latents = hand_video_latents.to(latents.device, latents.dtype)
            hand_video_latents = rearrange(hand_video_latents, "b c f h w -> b f c h w")

            hand_video_latents = torch.cat([hand_video_latents] * 2) if do_classifier_free_guidance else hand_video_latents

            inpaint_latents = torch.cat([inpaint_latents, hand_video_latents], dim=2).to(latents.dtype)
