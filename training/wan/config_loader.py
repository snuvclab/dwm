...
class ExperimentConfigLoader:
    ...
    def __init__(self, config_dir: Optional[Path] = None):
        ...
        self.config_dir = Path(config_dir) if config_dir is not None else self.repo_root / "configs"
    ...
    def validate_config(self, config: Dict[str, Any]) -> bool:
        ...
        if "pipeline" in config and "type" in config["pipeline"]:
            ...
            supported_pipeline_types = [
                ...
                "wan2.1_fun_inp_hand_concat_i2v_diffusers",
                "wan2.2_fun_inpaint",
    ...
    def resolve_repo_relative_paths(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in ("dataset_file", "validation_set", "exclude_videos_file"):
            value = data_config.get(key)
            if value is None:
                continue
            data_config[key] = self._resolve_repo_relative_path_values(value)

        return resolved

    def _resolve_repo_relative_path_values(self, value: Any) -> Any:
        if isinstance(value, str):
            return self._resolve_repo_relative_path(value)
        if isinstance(value, list):
            return [self._resolve_repo_relative_path(item) if isinstance(item, str) else item for item in value]
        return value

    def _resolve_repo_relative_path(self, raw_path: str) -> str:
        path = Path(raw_path)
        if path.is_absolute():
            return str(path)
...
def load_experiment_config(experiment_file: str, overrides: Optional[list] = None) -> Dict[str, Any]:
    ...
    if overrides:
        ...
        config = config_loader.resolve_repo_relative_paths(config)
