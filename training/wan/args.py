...
def _get_training_mode_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="lora",
        choices=["lora", "sft", "vace", "vace_lora"],
        help="Training mode: lora, sft, vace, or vace_lora (VACE modules full train + backbone LoRA).",
    )
    # LoRA parameters
...
def _get_training_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help="Training prompt mode. 'empty' ignores text conditioning during training.",
    )
    parser.add_argument(
        "--i2v_condition_latent_type",
        type=str,
        default="image",
        choices=["image", "fun_inp"],
        help="Which precomputed I2V conditioning latent to use when load_tensors=True.",
    )
...
def _get_checkpoint_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="How many latest checkpoints keep full accelerator state for exact resume.",
    )
...
def _get_validation_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="original",
        choices=["original", "empty", "both"],
        help="Validation prompt mode: use dataset prompt, empty prompt, or both.",
    )
