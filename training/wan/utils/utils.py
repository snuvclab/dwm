...
def get_video_to_video_latent(input_video_path, video_length, sample_size, fps=None, validation_video_mask=None, ref_image=None):
    if input_video_path is not None:
        ...
        if validation_video_mask is not None:
            ...
            input_video_mask = torch.from_numpy(iio.imread(validation_video_mask)).permute(3, 0, 1, 2)[:1].unsqueeze(0)
            input_video_mask = input_video_mask.to(input_video.device, input_video.dtype)
        else:
            input_video_mask = torch.zeros_like(input_video[:, :1])
            # input_video_mask[:, :, :] = 255  # keep the original video
            # input_video_mask[:, :, 1:] = 255  # I2V scenario: keep first frame
    else:
