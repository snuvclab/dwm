        ...
        if low_cpu_mem_usage:
            try:
                ...
                if pretrained_model_path.endswith(".safetensors"):
                    ...
                    with safe_open(pretrained_model_path, framework="pt", device="cpu") as reader:
                        checkpoint_keys = set(reader.keys())
                        for key, expected_tensor in model_state.items():
                            ...
                            if tuple(value.shape) != tuple(expected_tensor.shape):
                                raise ValueError(
                                    f"Size mismatch for {key}: checkpoint={tuple(value.shape)}, "
                            ...
                            set_module_tensor_to_device(
                                ...
                                param_device,
                                value=value,
                                dtype=torch_dtype if (value.is_floating_point() or value.is_complex()) else None,
                            )
                else:
                ...
                if missing_keys:
                    raise ValueError(
                        f"Cannot load {cls} from {pretrained_model_path} because the following keys are missing: "
                        + ", ".join(missing_keys)
                    )
