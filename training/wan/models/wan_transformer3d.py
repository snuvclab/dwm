...
def _should_cast_state_tensor(tensor: torch.Tensor) -> bool:
    return tensor.is_floating_point() or tensor.is_complex()


def _expand_patch_embedding_tensor(
    checkpoint_tensor: torch.Tensor,
    expected_shape: torch.Size,
) -> torch.Tensor:
    if tuple(checkpoint_tensor.shape) == tuple(expected_shape):
        return checkpoint_tensor

    if (
        checkpoint_tensor.ndim == len(expected_shape)
        and checkpoint_tensor.ndim >= 3
        and checkpoint_tensor.shape[0] == expected_shape[0]
        and tuple(checkpoint_tensor.shape[2:]) == tuple(expected_shape[2:])
        and checkpoint_tensor.shape[1] <= expected_shape[1]
    ):
        expanded = checkpoint_tensor.new_zeros(expected_shape)
        expanded[(slice(None), slice(0, checkpoint_tensor.shape[1]), *([slice(None)] * (checkpoint_tensor.ndim - 2)))] = checkpoint_tensor
        return expanded

    raise ValueError(
        "patch_embedding.weight shape mismatch: "
        f"checkpoint={tuple(checkpoint_tensor.shape)}, expected={tuple(expected_shape)}"
    )


def _initialize_missing_multiview_modules(
    model: nn.Module,
    missing_keys,
    ...
    for block_idx, block in enumerate(getattr(model, "blocks", [])):
        mixer = getattr(block, "cross_view_mixer", None)
        if mixer is None:
            continue

        prefix = f"blocks.{block_idx}.cross_view_mixer."
        block_missing = {key for key in missing_keys if key.startswith(prefix)}
        if not block_missing:
            continue

        mixer_cls = mixer.__class__
        mixer_eps = getattr(getattr(mixer, "norm", None), "eps", 1e-6)
        fresh_mixer = mixer_cls(
            dim=int(mixer.dim),
            num_heads=int(mixer.num_heads),
            eps=float(mixer_eps),
        ).to(dtype=torch_dtype)
        block.cross_view_mixer = fresh_mixer
        initialized.update(block_missing)
        ...
        if low_cpu_mem_usage:
            try:
                ...
                if os.path.exists(model_file):
                    ...
                    safetensor_files = [model_file_safetensors]
                    ...
                    with ExitStack() as stack:
                        ...
                        for safetensor_file in safetensor_files:
                            reader = stack.enter_context(
                                safe_open(safetensor_file, framework="pt", device="cpu")
                            )
                            for key in reader.keys():
                                if key in key_to_reader:
                                    raise ValueError(
                                        f"Duplicate key '{key}' found across safetensors files."
                                    )
                                key_to_reader[key] = reader
                        ...
                        for key, expected_tensor in model_state.items():
                            ...
                            if key == "patch_embedding.weight":
                                value = _expand_patch_embedding_tensor(value, expected_tensor.shape)
                            elif tuple(value.shape) != tuple(expected_tensor.shape):
                            ...
                            target_dtype = torch_dtype if _should_cast_state_tensor(value) else None
                    ...
                    initialized_keys = _initialize_missing_multiview_modules(
                        model,
                        missing_keys,
                        torch_dtype,
                    )
                    remaining_missing_keys = [
                        key for key in missing_keys if key not in initialized_keys
                    ]
                    ...
                    if meta_keys:
                        raise ValueError(
                            "The following tensors are still on the meta device after streaming load: "
                            + ", ".join(meta_keys[:20])
                            + (" ..." if len(meta_keys) > 20 else "")
                        )

                    unexpected_keys = list(checkpoint_keys - set(model_state.keys()))
                    if cls._keys_to_ignore_on_load_unexpected is not None:
                        for pat in cls._keys_to_ignore_on_load_unexpected:
                            unexpected_keys = [
                                key for key in unexpected_keys if re.search(pat, key) is None
                            ]

                    if size_mismatch_keys:
                        for key, found_shape, expected_shape in size_mismatch_keys:
                            print(
                                f"{key} Size don't match, skip. "
                                f"checkpoint={found_shape}, expected={expected_shape}"
                            )

                    if remaining_missing_keys:
                        print(f"### missing keys: {len(remaining_missing_keys)};")
                        print(remaining_missing_keys)
                    if unexpected_keys:
                        print(
