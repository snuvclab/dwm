...
def main():
    ...
    parser.add_argument("--gt_data_root", type=str, required=True, help="Root directory containing data.")
    parser.add_argument("--out_root", type=str, required=True, help="Root directory to save output.")
    parser.add_argument("--name", type=str, required=True, help="Name of the sequence.")
    args = parser.parse_args()

    gt_data_root = Path(args.gt_data_root)
    ...
    # GT data
    with open(gt_data_root / "trajectory" / f"{args.name}.npy", "rb") as f:
        gt_trajectory = np.load(f)
    gt_intrinsics = np.load(gt_data_root.parent / "cam_params" / "intrinsics.npy")
    gt_frames = iio.imread(gt_data_root / "videos" / f"{args.name}.mp4")

    gt_disparities = np.load(gt_data_root / "disparity" / f"{args.name}.npz")["disparity"]
    gt_raymaps = np.load(gt_data_root / "raymaps" / f"{args.name}.npz")["raymap"]
    gt_human_motions = np.load(gt_data_root / "human_motions" / f"{args.name}.npz")

    # Calculate point maps using disparities and raymaps
    point_maps = postprocess_pointmap(
        disparity= gt_disparities,
        raymap=gt_raymaps,
        ray_o_scale_inv=0.1,
    )["pointmap"]
    ...
    # output data
    task = ""
    out_raymaps = np.load(out_root / f"{args.name}{task}_raymap.npy")
    out_human_motion = np.load(out_root / f"{args.name}{task}_human_motion.npy")

    # out_disparities = np.load(out_root / f"{args.name}_{task}_disparity.npy")
    out_frames = iio.imread(out_root / f"{args.name}{task}.mp4")

    outputs = postprocess_pointmap(
        disparity= gt_disparities,
        raymap=out_raymaps,
        ray_o_scale_inv=0.1,
    )
    out_trajectory = outputs["camera_pose"]
    out_intrinsics = outputs["intrinsics"][0]
    out_human_motions_smplh = {
        "body_pose_quat": matrix_to_quaternion(rotation_6d_to_matrix(torch.tensor(out_human_motion[:, :126].reshape(49, -1, 6)))),
        "left_hand_pose_quat": matrix_to_quaternion(rotation_6d_to_matrix(torch.tensor(out_human_motion[:, 126:216].reshape(49, -1, 6)))),
        "right_hand_pose_quat": matrix_to_quaternion(rotation_6d_to_matrix(torch.tensor(out_human_motion[:, 216:].reshape(49, -1, 6)))),
    }

    
    # Create the SMPLH model
    model_path = 'data_processing/egoallo/data/smplh/neutral/model.npz'
    device = "cuda"
    ...
    for i in tqdm(range(num_frames)):
        ...
        # Add gt SMPLH mesh
        smplh_shape = smplh_model.with_shape(betas=torch.tensor(gt_human_motions['betas'][i, :], device=device))

        local_quats = torch.cat([
            torch.tensor(gt_human_motions['body_pose_quat'][i, :], device=device),
            torch.tensor(gt_human_motions['left_hand_pose_quat'][i, :], device=device),
            torch.tensor(gt_human_motions['right_hand_pose_quat'][i, :], device=device),
        ], dim=0)
        ...
        def transform_fn(pose_quat):
            return matrix_to_quaternion(rotation_6d_to_matrix(matrix_to_rotation_6d(quaternion_to_matrix(pose_quat))))


        # Add output camera poses
        fov = 2 * np.arctan2(frame.shape[0] / 2, gt_intrinsics[0, 0])
        aspect = frame.shape[1] / frame.shape[0]
        server.scene.add_camera_frustum(
            f"/frames/t{i}/frustum_out",
            fov=fov,
            aspect=aspect,
            scale=0.15,
            image=frame_out[::4, ::4],
            wxyz=tf.SO3.from_matrix(out_trajectory[i][:3, :3]).wxyz,
            position=out_trajectory[i][:3, 3],
        )
        # Add some axes.
        server.scene.add_frame(
            f"/frames/t{i}/frustum_out/axes",
            axes_length=0.3,
            axes_radius=0.01,
            origin_color=(1, 0, 0),  # black for ground truth
        )

        # Add output SMPLH mesh
        local_quats_out = torch.cat([
            transform_fn(torch.tensor(out_human_motions_smplh['body_pose_quat'][i, :], device=device)),
            transform_fn(torch.tensor(out_human_motions_smplh['left_hand_pose_quat'][i, :], device=device)),
            transform_fn(torch.tensor(out_human_motions_smplh['right_hand_pose_quat'][i, :], device=device)),
        ], dim=0)
        smplh_pose_out = smplh_shape.with_pose(T_world_root=torch.tensor([1, 0, 0, 0, 0, 0, 0], device=device, dtype=torch.float32),
                                        local_quats=local_quats_out)
        cpf_pos_out = torch.tensor((tf.SE3.from_matrix((out_trajectory[i]))).wxyz_xyz, dtype=torch.float32, device=device)
        cpf_pos_out = compose_se3_qwxyz(cpf_pos_out, torch.tensor([0, 0, 0, 1, 0, 0, 0], device=device, dtype=torch.float32))
        T_world_root_out = fncsmpl_extensions.get_T_world_root_from_cpf_pose(smplh_pose_out, cpf_pos_out)
        smplh_pose_out = smplh_shape.with_pose(T_world_root=T_world_root_out,
                                        local_quats=local_quats_out)
        mesh_out = smplh_pose_out.lbs()
        vertex_colors_out = np.array([255, 0, 0])  # Red for output
        mesh_out = trimesh.Trimesh(vertices=mesh_out.verts.cpu().numpy() * 1,
                                faces=mesh_out.faces.cpu().numpy(),
                                vertex_colors=np.repeat(vertex_colors_out[None, :], mesh_out.verts.shape[0], axis=0))
        smpl_nodes.append(
            server.scene.add_mesh_trimesh(
                f"/frames/t{i}/smplh_out",
                mesh=mesh_out,
            )
        )
