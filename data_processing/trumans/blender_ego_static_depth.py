...
# Depth-only script; we'll set file_format per write_still call
...
# Create solid diffuse material for depth override
...
# No compositor output needed; we'll save depth directly via write_still and render.filepath
...
def render_animation_sequence(animation_index, animation_name):
    ...
    depth_output_path  = os.path.join(sequences_folder, "depth_static")
    ...
    # Depth: first frame per clip only
    try:
    ...
    for video_idx, start_frame_num in enumerate(video_start_frames):
        for video_idx, start_frame_num in enumerate(video_start_frames):
            depth_exr_path = os.path.join(depth_output_path, f"{video_idx:05d}.exr")
            def file_ok(path, min_size=1024):
                ...
                try:
                    ...
                    return False
            if args.skip_existing and file_ok(depth_exr_path, min_size=102400):
                print(f"SKIPPED Depth: {os.path.basename(depth_exr_path)} exists")
                continue

            # Prepare static scene at start frame
            ...
            # Switch to DepthSolid layer and save directly via render.filepath
            try:
                bpy.context.window.view_layer = depth_layer
            except Exception:
                try:
                    # Fallback to context view_layer if window attribute missing
                    bpy.context.view_layer = depth_layer
                except Exception:
                    pass
            orig_format = render.image_settings.file_format
            render.image_settings.file_format = 'OPEN_EXR'
            render.filepath = os.path.join(depth_output_path, f"{video_idx:05d}")
            try:
                bpy.ops.render.render(write_still=True)
                print(f"  ✅ Saved depth EXR: {os.path.basename(depth_exr_path)}")
            finally:
                ...
                try:
                    ...
                    try:
                        bpy.context.view_layer = current_view_layer
                    except Exception:
                        pass
    ...
    print(f"Depth frames created: {len(video_start_frames)}")
