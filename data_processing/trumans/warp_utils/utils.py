...
def save_video(data, images_path, folder=None, fps=8):
    ...
    if isinstance(data, np.ndarray):
        # Data is already in [0, 1] range, just ensure float32
        video_data = data.astype(np.float32)
    elif isinstance(data, torch.Tensor):
        # Convert to numpy, data is already in [0, 1] range
        video_data = data.detach().cpu().numpy().astype(np.float32)
        ...
        # Images from file are in [0, 255] range, normalize to [0, 1]
        video_data = stacked_images.astype(np.float32) / 255.0
    
    # Ensure output path has .mp4 extension
    if not str(images_path).endswith('.mp4'):
        images_path = str(images_path) + '.mp4'
    
    try:
        export_to_video(video_data, images_path)
        print(f"✅ Video saved successfully: {images_path}")
    except Exception as e:
        print(f"❌ Error saving video with export_to_video: {e}")
        # Fallback to torchvision if export_to_video fails
        try:
            tensor_data = torch.from_numpy((video_data * 255).astype(np.uint8))
            torchvision.io.write_video(
                images_path, tensor_data, fps=fps, video_codec='libx264'
            )
            print(f"✅ Video saved with torchvision fallback: {images_path}")
...
class Warper:
    ...
    def forward_warp(
        ...
        if not twice:
            ...
            # Warp the depth map as well
            warped_depth2, _ = self.bilinear_splatting(
                depth1, mask1, trans_depth1, flow12, None, is_image=False
            )
            if mask:
            ...
            return twice_warped_frame1, warped_frame2, warped_depth2, None
...
def colorize_depth(depth, cmap="Spectral"):
    ...
    depth = 1 - depth  # Invert so closer objects are brighter
    
    cm = cmaps.get_cmap(cmap)
    depth = depth.clip(0, 1)
    depth_colored = cm(depth, bytes=False)[..., 0:3]
