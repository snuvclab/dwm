        ...
        if N <= num_frames or N <= num_interp_frames + 2 - num_overlap_frames:
            ...
            for i in range(0, N, num_interp_frames + 2 - num_overlap_frames):
                if (
                    ...
                    key_frame_indices.append(N-1-num_interp_frames - 1)
                    key_frame_indices.append(N-1)
                    break
                key_frame_indices.append(i)
                key_frame_indices.append(i + num_interp_frames + 1)
                if i + num_interp_frames + 1 == N - 1:
                    break 
            ...
            for i in tqdm.tqdm(range(0, len(key_frame_indices), 2)):
                ...
                masked_depth_latent = torch.zeros(
                    (B, key_frame_indices[i + 1] - key_frame_indices[i] + 1, *key_depth_latent.shape[2:])
                ).to(key_depth_latent)
                ...
                if pre_latent is not None:
                    ...
                    overlap_b = latent[:, : pre_latent.shape[1] - key_frame_indices[i]]
                    assert overlap_a.shape[1] == overlap_b.shape[1]
                    ratio = (
                        torch.linspace(0, 1, overlap_a.shape[1])
                        .to(overlap_a)
                        .view(1, -1, 1, 1, 1)
                    )
                    overlap = overlap_a * (1 - ratio) + overlap_b * ratio
                    pre_latent[:, key_frame_indices[i] :] = overlap
                    pre_latent = torch.cat([pre_latent, latent[:, pre_latent.shape[1] - key_frame_indices[i]:]], dim=1)
                else:
                    pre_latent = latent

                torch.cuda.empty_cache()

            depth_latent = pre_latent
