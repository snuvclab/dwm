#!/bin/bash
#SBATCH --job-name=hawor_cam_only
#SBATCH --nodes=1
#SBATCH --gpus=4
#SBATCH --partition=jhb_vclab
#SBATCH --output=out/251105/%j_hawor_cam_only.out
#SBATCH --error=out/251105/%j_hawor_cam_only.err
...
cleanup() {
    ...
    pkill -f "process_hawor_batch_cam_only.py" 2>/dev/null
...
# Change to HaWoR directory for library imports
cd $HOME/world_model/data_processing/HaWoR
...
# Configuration (paths relative to world_model root)
TRAIN_LIST="$HOME/world_model/data/taste_rob/double_resized/train_list.txt"
...
SKIP_EXISTING=true
FPS=""  # Empty means auto-detect from video
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --fps N                  Frames per second for video extraction (default: auto-detect)"
            ...
            echo ""
            echo "Note: This script uses process_hawor_batch_cam_only.py (no SLAM/infiller)"
            exit 0
            ;;
        *)
...
echo "===== HaWoR Batch Processing (Camera Space Only) ====="
...
echo "FPS: ${FPS:-auto-detect}"
echo "Skip existing: $SKIP_EXISTING"
echo "Note: Using camera space only (no SLAM/infiller)"
echo ""
...
# Build command (relative to HaWoR directory)
SCRIPT_NAME="process_hawor_batch_cam_only.py"
...
if [ "$DEBUG_MODE" = true ]; then
    ...
    echo "===== Running HaWoR Batch Processing (Camera Space Only) ====="
...
echo ""
echo "===== HaWoR Batch Processing (Camera Space Only) completed ====="
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "   Max workers per GPU: $MAX_WORKERS_PER_GPU"
    echo "   Skip existing: $SKIP_EXISTING"
    echo "   Mode: Camera space only (no SLAM/infiller)"
    echo ""
    ...
    echo "   - Verify checkpoint path"
    echo "   - Try debug mode with --debug"
fi

echo ""
echo "📁 SLURM output files:"
echo "   - Output: out/251105/%j_hawor_cam_only.out"
echo "   - Error: out/251105/%j_hawor_cam_only.err"
